import Card from "./Card";
import {
  createArmorCardPlay,
  createAttackCardPlay,
  createDrawCardPlay,
} from "../actions/playerCardActions";

const playerCardsData = [
  new Card({
    id: "player-card-1",
    name: "Quick Strike",
    type: "player",
    damage: 2,
    armor: 2,
    play(gameState) {
      let effectiveDamage = this.damage;
      let nextPendingMonsterEffect = gameState.pendingMonsterEffect;
      let armorReward = 0;

      if (
        gameState.pendingMonsterEffect?.effect === "playerMaxDamageNextTurn"
      ) {
        effectiveDamage = Math.min(
          this.damage,
          gameState.pendingMonsterEffect.value
        );
        nextPendingMonsterEffect = null;
      }

      if (effectiveDamage === 2) {
        armorReward = 2;
      }

      const updatedHand = gameState.player.hand.filter(
        (card) => card.id !== this.id
      );

      const updatedMonsterDeck =
        gameState.currentMonster.deck.slice(effectiveDamage);

      const updatedPlayerArmor = Math.min(
        4,
        gameState.player.armor + armorReward
      );

      return {
        ...gameState,
        pendingMonsterEffect: nextPendingMonsterEffect,
        player: {
          ...gameState.player,
          hand: updatedHand,
          armor: updatedPlayerArmor,
        },
        currentMonster: {
          ...gameState.currentMonster,
          deck: updatedMonsterDeck,
        },
      };
    },
  }),

  new Card({
    id: "player-card-2",
    name: "Shield Up",
    type: "player",
    armor: 1,
    play: createArmorCardPlay(1),
  }),

  new Card({
    id: "player-card-3",
    name: "Battle Focus",
    type: "player",
    draw: 2,
    play: createDrawCardPlay(2),
  }),

  new Card({
    id: "player-card-4",
    name: "Light Focus",
    type: "player",
    draw: 1,
    play: createDrawCardPlay(1),
  }),

  new Card({
    id: "player-card-5",
    name: "Deep Focus",
    type: "player",
    draw: 2,
    play: createDrawCardPlay(2),
  }),

  new Card({
    id: "player-card-6",
    name: "Sword Slash",
    type: "player",
    damage: 1,
    play: createAttackCardPlay(1),
  }),

  new Card({
    id: "player-card-7",
    name: "Strong Attack",
    type: "player",
    damage: 2,
    play: createAttackCardPlay(2),
  }),

  new Card({
    id: "player-card-8",
    name: "Guard Stance",
    type: "player",
    armor: 1,
    play: createArmorCardPlay(1),
  }),

  new Card({
    id: "player-card-9",
    name: "Tactical Move",
    type: "player",
    draw: 1,
    play: createDrawCardPlay(1),
  }),

  new Card({
    id: "player-card-10",
    name: "Battle Insight",
    type: "player",
    draw: 2,
    play: createDrawCardPlay(2),
  }),
];

export default playerCardsData;
